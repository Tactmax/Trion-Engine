...
- Light-only entities resolve to their runtime light object (`LightSystem.getLight()`), whose orientation mirrors the ECS transform so the same `TransformControls` gizmo drives light position/direction; selection picks a directional light in rotate mode and point/spot lights in translate mode. Viewport picking raycasts a detached proxy sphere at each unmeshed light's world position. Gizmo drags write the `TransformComponent` through the existing history path, so undo/redo and Play Mode snapshot/restore apply unchanged.
